...
class TestWriteManifest:
    def test_writes_header_and_metadata_rows(self, tmp_path):
        ...
        assert rows[0] == [
            ...
            "ORIGINAL_START_TIME", "ORIGINAL_END_TIME", "ARTIST", "SONG",
        ]
        assert rows[1] == ["1", "r_01_a.mp4", "0:00", "2:00", "Elephant Gym", "Dear Humans"]
...
class TestComputeMusicDensity:
    def test_all_music_is_dense(self):
        times, densities = compute_music_density([("music", 0, 100)], 100, window_size=20, step=5)
        assert len(times) == len(densities)
        assert densities.max() <= 1.0
        assert densities.max() > 0.9

    def test_no_music_is_zero_density(self):
        _, densities = compute_music_density([("speech", 0, 100)], 100, window_size=20, step=5)
...
class TestFindSongRegions:
    def test_detects_single_region(self):
        ...
        regions = find_song_regions(times, densities, threshold=0.15, min_song_duration=30)
...
class TestParseDescriptionTimestamps:
    def test_parses_mmss(self):
        ...
        assert parse_description_timestamps(desc) == [
            (0, "Intro"), (225, "Song One"), (630, "Song Two"),
        ]

    def test_parses_hhmmss(self):
        assert parse_description_timestamps("1:02:03 - Late Song") == [(3723, "Late Song")]
    ...
    def test_hhmmss_falls_back_to_mmss_when_beyond_duration(self):
        ...
        assert parse_description_timestamps("1:30:00 - Weird", total_duration=600) == [(90, "Weird")]
...
class TestSelectBestSplits:
    def test_keeps_most_balanced_splits(self):
        ...
        assert select_best_splits(splits, total_duration=300, n_songs=3) == [100.0, 200.0]
